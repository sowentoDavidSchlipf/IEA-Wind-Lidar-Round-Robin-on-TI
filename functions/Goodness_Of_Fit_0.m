function [correlation_coefficient,a,b,N,...
    regression_string,correlation_string,N_string] = Goodness_Of_Fit_0(x,y,type)
...
% fit_parameters = polyfit(x,y,0);
% a = fit_parameters(1);
b = 0;

y_eq_ax   = @(a,xdata)(a(1)*xdata);
x0         = [1];
a          = lsqcurvefit(y_eq_ax,x0,x,y);



regression_string=sprintf('%s%.2f%s%.2f','y = ',a,'x ');
